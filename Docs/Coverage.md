...
| `Network`, `Network.TCP`, `Network.UDP`, `Network.TLS` | IP parse/format, endpoint format, resolve; both TCP connect forms with bounded connect, read and write waits, listen/accept, endpoints, stream IO, half-close and close; UDP open/bind/endpoints/send/receive/close; TLS provider availability, certificate-verifying connect or upgrade of an owned TCP stream, stream IO and close | all programs under `Examples/Network` | `Tests/Network.sx`, `Tests/NetworkSockets.sx`, `Tests/NetworkTLS.sx` |
| `Process`, `Subprocess`, `System` | arguments, pid, current directory mutation, executable path; captured commands and interactive children; scoped environment, partial input writes, timed output/error events, termination, exit status and capture limits; platform, architecture and target naming | all programs under `Examples/Process`, `Examples/Subprocess`, `Examples/System` | `Tests/Process.sx`, `Tests/Subprocess.sx`, `Tests/System.sx` |
