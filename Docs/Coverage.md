...
| `Network`, `Network.TCP`, `Network.UDP`, `Network.TLS` | IP parse/format, endpoint format, resolve; both TCP connect forms, listen/accept, endpoints, stream IO, half-close and close; UDP open/bind/endpoints/send/receive/close; TLS provider availability, certificate-verifying connection, stream IO and close | all programs under `Examples/Network` | `Tests/Network.sx`, `Tests/NetworkSockets.sx`, `Tests/NetworkTLS.sx` |
