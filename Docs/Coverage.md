...
| `Collections.Dictionary`, `Set`, `Queue`, `Stack`, `Hashing` | all constructors; count, capacity, reserve, clear, containment, mutation, lookup, removal and complete iteration; dictionary key/value snapshots; built-in and custom hashing/equality | recipes under `Docs/Recipes/Collections` | `Tests/Collections.sx` |
| `Console`, `Console.Session` | standard/error output, flush, TTY detection, dimensions, colors, styles, cursor/screen control, line input, Enter wait, blocking and timed key input, bounded draining, alternate screen and restoration | recipes under `Docs/Recipes/Console` | `Tests/Console.sx` |
| `Compression` | bounded raw deflate, gzip and zlib decompression; malformed stream, checksum and output-limit failures | HTTP content decoding consumers | `Tests/Compression.sx` |
| `Crypto`, `Crypto.X25519`, `Crypto.HKDF`, `Crypto.ChaCha20Poly1305` | secure random bytes; MD5, SHA-1 and SHA-256 for text and bytes; hexadecimal and byte digests; X25519 key generation and shared secrets; HKDF-SHA256 derivation; authenticated ChaCha20-Poly1305 sealing, opening and tamper rejection | `ContentFingerprint`, `HashFile`, `AuthenticatedMessage` | `Tests/Crypto.sx`, `Tests/CryptoPrimitives.sx` |
...
| `File` | every access, creation and seek mode; open/close, read/write/flush, position, length, resize, complete text and byte IO with limits | recipes under `Docs/Recipes/Files` | `Tests/File.sx`, `Tests/IO.sx` |
...
| `FileWatch` | recursive and direct-child selection; created, modified, removed, renamed and metadata notifications; bounded waits, explicit close and rescan recovery signal | `WatchWorkspace` | `Tests/FileWatch.sx`; native execution on macOS and target composition on Linux/Windows |
| `Regex` | structured compilation; cached `Presets` for digits, email, IPv4 and IPv6; intention-based `contains`, `first`, lazy `find`, whole-text `match` and eager `all`; leftmost-first matching; Unicode properties, word boundaries and scalar ranges; greedy/lazy quantifiers; numbered/named/noncapturing groups; case, multiline and dot-all options; split and literal/callback replacement; empty-match progress; large streaming input and compact finite bounds | `ExtractContacts`, `NormalizeLog`, `UsePresets` | `Tests/Regex.sx`, [`RegexStreamingSearch`](https://github.com/Matanek/Silex-Benchmarks/blob/main/Sources/RegexStreamingSearch.sx) |
...
| `Network`, `Network.TCP`, `Network.UDP`, `Network.TLS` | IP parse/format, endpoint format, resolve; both TCP connect forms with bounded connect, read and write waits, listen/accept, endpoints, stream IO, half-close and close; UDP open/bind/endpoints/send/receive/close; TLS provider availability, certificate-verifying connect or upgrade of an owned TCP stream, stream IO and close | recipes under `Docs/Recipes/Network` | `Tests/Network.sx`, `Tests/NetworkSockets.sx`, `Tests/NetworkTLS.sx` |
| `Process`, `Subprocess`, `System` | arguments, pid, current directory mutation, executable path; captured commands and interactive children; scoped environment, partial input writes, timed output/error events, termination, exit status and capture limits; platform, architecture and target naming | recipes under `Docs/Recipes/Process`, `Docs/Recipes/Subprocess`, `Docs/Recipes/System` | `Tests/Process.sx`, `Tests/Subprocess.sx`, `Tests/System.sx` |
...
| `Threading` | executor constructors, typed completion, fences, completion observation, single/multiple dependencies, combine, parallel ranges, delayed parallel work and idle wait | recipes under `Docs/Recipes/Threading` | `Tests/Threading.sx`, `Tests/ThreadingParallelOnly.sx` |
